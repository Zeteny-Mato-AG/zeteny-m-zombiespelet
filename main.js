...
function update() {
    ...
    for (i = 0; i < zombies.length; i++) {
        ...
        rectangle(zombies[i].x - 25, zombies[i].y - 50, zombies[i].health * 10, 6, "green")

    }
    for (i = 0; i < zombies.length; i++) {
        if (zombies[i].health == 0) {
            score++
            zombies.splice(i, 1, { x: random(600), y: random(600), health: 6, iFrames: 0 })
        }
    }
    ...
    for (var i = 0; i < bullet.length; i++) {
        rectangle(bullet[i].x, bullet[i].y, bullet[i].size, bullet[i].size, "yellow")
